...
use openreel_core::{BatchError, Document, Operation, TimelineRevision, apply_batch};
use rmcp::model::Tool;
...
mod tests {
    ...
    fn prepared_plan_store_consumes_a_plan_once() {
        ...
        document.tracks.push(openreel_core::Track {
            id: openreel_core::TrackId(1),
            kind: openreel_core::TrackKind::Video,
            sync_lock: true,
            clips: Vec::new(),
        });
        let mut store = PreparedPlanStore::default();
