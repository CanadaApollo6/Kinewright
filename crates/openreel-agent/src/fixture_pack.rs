...
const USER_AGENT: &str = "OpenReel-Eval-Fixture/0.1 (+https://github.com/CanadaApollo6/OpenReel)";
...
pub enum FixturePackError {
    ...
    UnknownAsset(String),
    #[error(
        "fixture asset {asset:?} is missing at {path}; run openreel-eval --prepare-fixtures with the pack manifest first"
    )]
    ...
    HashAsset {
        asset: String,
        path: PathBuf,
        source: openreel_core::MediaError,
    },
}
...
pub fn fixture_cache_root() -> PathBuf {
    env::var_os("OPENREEL_EVAL_FIXTURE_DIR").map_or_else(
        || {
...
fn verify_asset(asset: &FixtureAsset, path: &Path) -> Result<(), FixturePackError> {
    ...
    let hash = openreel_media::sha256_file(path).map_err(|source| FixturePackError::HashAsset {
        asset: asset.id.clone(),
        path: path.to_path_buf(),
        source,
    })?;
...
mod tests {
    ...
    fn temporary_root(label: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        env::temp_dir().join(format!("openreel-{label}-{}-{nonce}", std::process::id()))
    }
