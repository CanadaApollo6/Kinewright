    ...
    fn send_user_message(&mut self, text: String) -> Result<(), AgentError> {
            ...
            .is_err()
        {
            return Err(AgentError::Unavailable(
                "another Kinewright Cursor turn is active; wait for it to finish or stop it"
                    .to_owned(),
            ));
        }
        ...
        thread::Builder::new()
            .name("kinewright-cursor-turn".to_owned())
            .spawn(move || {
...
fn spawn_cursor_incoming(
    ...
    thread::Builder::new()
        .name("kinewright-cursor-events".to_owned())
        .spawn(move || {
