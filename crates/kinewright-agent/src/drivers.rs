    ...
    fn start_session(&self, cfg: SessionConfig) -> Result<Box<dyn AgentSession>, AgentError> {
        ...
        let version = codex_process_output(&target, &["--version"]).ok_or_else(|| {
            AgentError::Unavailable(
                "Kinewright could not verify the installed Codex CLI version; refusing to launch it"
                    .to_owned(),
            )
        })?;
        if !codex_version_is_supported(&version) {
            return Err(AgentError::Unavailable(format!(
                "Codex CLI 0.147.0 or newer is required for Kinewright's restricted driver (found {})",
                version.trim()
            )));
        }
...
impl ClaudeSession {
    fn spawn(executable: PathBuf, endpoint: &str, cfg: &SessionConfig) -> Result<Self, AgentError> {
        let tool_allowlist = configured_tool_names(cfg)
            .into_iter()
            .map(|name| format!("mcp__kinewright__{name}"))
            .collect::<Vec<_>>()
            .join(",");
        let mcp_config = json!({
            "mcpServers": {
        ...
        if let Some(stderr) = child.stderr.take() {
            thread::Builder::new()
                .name("kinewright-claude-stderr".to_owned())
                .spawn(move || {
...
fn build_codex_command(
    ...
    command
        ...
        .arg("-c")
        .arg(format!("mcp_servers.kinewright.url={endpoint}"))
        .arg("-c")
        .arg(format!("mcp_servers.kinewright.enabled_tools={tool_names}"))
        .arg("-c")
        .arg("mcp_servers.kinewright.required=true")
        .arg("-c")
        .arg("mcp_servers.kinewright.default_tools_approval_mode='approve'")
        .arg(prompt)
...
fn create_codex_direct_model_catalog(
    ...
    let mut catalog = cached.or_else(bundled).ok_or_else(|| {
        AgentError::Unavailable(
            "Kinewright could not load Codex model metadata to force direct MCP tool calling"
                .to_owned(),
        )
    })?;
...
fn spawn_claude_reader(
    ...
    thread::Builder::new()
        .name("kinewright-claude-events".to_owned())
        .spawn(move || {
...
mod tests {
    ...
    fn codex_command_is_read_only_and_has_an_exact_mcp_allowlist() {
        ...
        assert!(joined.contains("mcp_servers.kinewright.required=true"));
        assert!(joined.contains(
            "mcp_servers.kinewright.enabled_tools=[\"get_timeline_state\",\"split_clip\"]"
        ));
