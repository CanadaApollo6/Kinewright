...
$sourceExe = Join-Path $BundleDir 'OpenReel.exe'
...
$smokeDir = [System.IO.Path]::GetFullPath((Join-Path $tempRoot "OpenReel-bundle-smoke-$([guid]::NewGuid().ToString('N'))"))
...
try {
    ...
    $smokeExe = Join-Path $smokeDir 'OpenReel.exe'
    ...
    try {
        ...
        if (-not $process.Start()) {
            throw 'OpenReel.exe did not start.'
        }
    } finally {
    ...
    if ($process.WaitForExit($StartupTimeoutSeconds * 1000)) {
        throw "OpenReel.exe exited during the startup smoke window with code $($process.ExitCode)."
    }

    Write-Host "OpenReel.exe stayed running for $StartupTimeoutSeconds seconds from an isolated directory with a system-only PATH."
} finally {
