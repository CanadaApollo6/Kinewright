...
- **HDR, BT.2020, PQ (SMPTE 2084), and HLG (ARIB STD-B67).** The `ColorTransfer` and `ColorPrimaries` vocabularies already name them (`color.rs:69-100`) and `probe_path` already reads them on ingest; delivery keeps rejecting them with typed reasons. HDR delivery needs mastering-display and content-light metadata, a tone-mapping contract, and a monitoring path this project does not have. **Superseded in part by CC8 (accepted 2026-08-29; `docs/CC8-HDR-INTERPRETATION-AND-DELIVERY.md`).** CC8 takes HDR *interpretation* (two source profiles, `pq_rec2020` and `hlg_rec2020`, at 10 bits or more), a named non-identity Rec.2020↔BT.709 primaries conversion, and exactly one HDR delivery lane — HLG, Rec.2020, BT.2020 NCL, limited range, 10-bit, on this document's own libx264 High 10 lane — so "delivery keeps rejecting them" narrows to *every HDR description outside that one lane*, by CC8 §5.3's lane-derived widening of `delivery_color_mismatches`, not by a global rule. **What stays deferred, and why this bullet's reasoning still holds where it holds:** PQ/HDR10 delivery and with it mastering-display provenance and *gated* MaxCLL/MaxFALL (CC8 reads and reports the metadata, §2.4/§6, and writes none of it); dynamic metadata in every form — Dolby Vision, HDR10+, ST 2094; calibrated HDR monitoring, which is still the separate later programme this bullet names and which CC8's labelled tone-mapped preview is explicitly not a down payment on; and tone-mapped **delivery**, refused on this section's own gamut-mapping reasoning (the bullet above) — a rendering intent and a compression curve with no defensible objective threshold — restated by CC8 §0.2 Q6 on the luminance axis. Nothing in CC6's measured content is reopened: §5's budgets, §6.3's per-lane constants, and every pinned figure here are unchanged by CC8, whose §9.1 fixture 6 gates the SDR lanes byte-identical.
