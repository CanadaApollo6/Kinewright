...
- **Windows build.** The Windows CI job runs a *different* FFmpeg package from the Linux pin; both are named, and the Windows job is the Windows measurement (P11). **Amended 2026-09-12 (Riel; AU3 §0 E59 carries the normative rule).** Budget constants still carry a ≥ 2× margin and are still single and shared — a budget is never per-OS and never widened to span two systems — but the **measured column is now per operating system, each value measured on that system and optimized there**. *Was:* "budget constants carry a ≥ 2× margin over the Linux measurement rather than being per-OS", which made the Linux number stand in for a Windows one it had never been compared against; the first CI run that actually failed on a failing test showed the two builds disagree (CC7 scenario **a**'s eight-bit luma mean is 18 677 on Linux and 18 688 on Windows, and a flat neutral chroma plane dithers to two codes on Linux and one on Windows). A system with no column fails loudly rather than passing by omission, and no number is copied across systems. **Scenario f's ten-bit re-baseline — luma mean 281 → 274 and RGB mean 77 446 → 77 447, which PR #6's libx264 thread pin made deterministic — was approved by Riel on 2026-09-12 and stands.** **Measured outcome:** of CC7's 60 delivery rows, **44 read identically on both builds** and stay single values — which now means "measured on both and found equal", not "Linux's, assumed" — and **16 differ**, every one of them with Windows decoding *closer* to the source (lower RGB mean difference, higher PSNR). The Windows column and its provenance were harvested from CI run 34693346093 and carry their own margins.
