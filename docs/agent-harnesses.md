...
Transport: one long-lived `agent acp` child over ACP v1 NDJSON. Kinewright advertises no client
...
ACP `allow_once`; destructive Kinewright operations still stop at the same `ConfirmationBroker` used
...
even when invoked through ACP. Kinewright therefore snapshots the complete Cursor configuration,
...
configuration and Claude's exact tool allowlist. Kinewright's transactional operation validation,
...
`KINEWRIGHT_CURSOR_AGENT_TEST=1` gate so it can be run alone. Each creates a two-clip project,
