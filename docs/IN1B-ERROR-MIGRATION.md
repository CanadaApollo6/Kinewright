...
The words **must**, **must not** and **may** are normative. Rules are numbered inside their section so a critic can cite "§3 rule 12". Every code claim carries `file:line` at **`d4ed8eb`** and was re-measured 2026-09-15 by probe-2b and again by this revision. Revision 1's four `[probe-2b]` markers are all replaced by the measured value; **one** number remains unmeasured, written **[probe-2c]**, and §10 states its expected outcome.
...
4. **Per-label totals under the sink gate** are Look **17** and Media **8**, because the three bypasses carry the labels `"Look"` (`app.rs:760`) and `"Media"` (`app.rs:775`, `:1377`). Confirmed by probe-2b T6.
...
```
   ...
   Each family answers one question — *what must change for the same request to succeed?* — and each has a distinct recovery sentence, written in §5.6. Probe-1b P1 confirmed no family has an empty membership and no two share a sentence.
...
| --- | --- | --- |
| `Asset(AssetId)` | one media file that keeps refusing | Media, Relink, Source monitor, Look (per-LUT), Operations (per-asset) |
...
| `Track(TrackId)` | one track's capture refusal | **row 124** only (`timeline_ui.rs:3266`, `RoomToneCaptureJob::track`), plus the five track-addressed operations of rule 18 |
...
36. `MediaError` (`media.rs:1739`) gains `MatteProof(MatteProofError)` and `MatteCoverage(MatteCoverageError)`, both `#[error(transparent)]`, and the two `From` impls at `media.rs:600` and `:659` stop flattening into `Self::Backend(error.to_string())`. `recovery_code()` (`media.rs:1826`) gains two arms delegating to `MatteProofError::code()` (`media.rs:589`) and `MatteCoverageError::code()` (`media.rs:648`), so it returns `Some` for **8 of 13** variants instead of 6 of 11. The rendered text is unchanged: both `code()` strings are already the first token of each `#[error]` template, so `matte_coverage_invalid_dimensions: …` renders identically with or without the `media backend error: ` prefix that `Backend` used to add — **and the prefix is the one visible change**, which is why §7 requires a media test asserting the new rendering.

37. **Neither mints an `IncidentCode`** (§0.2/e). `IncidentObservation::from_media_error`'s exhaustive match (IN1 §2.3b rule 22) gains two arms returning `None`, each with the one-line reason: the app reads both through `Result<_, String>` seams that never reach any of the three sinks §1 claims — `matte_overlay_ui.rs:100` and `color_scopes_ui.rs:1073`, both named in §13 D-B3.
...
5. **The actor echoes and never invents.** `execute_command` (`actor.rs:385`) threads the token from the command into whichever event it returns — `RevisionConflict` from `revision_conflict` (`actor.rs:423`), `DocumentChanged` from `execute_operation` (`actor.rs:430`). Every other command path emits `token: None`, including the initial snapshot `DocumentChanged` at `actor.rs:361`. A core test **must** assert both directions: a `DoIfRevision` with a token that conflicts echoes that token on `RevisionConflict`, and one that lands echoes it on `DocumentChanged`.
...
| ---: | --- | ---: | --- |
| 1 | `MediaError`'s sites and their siblings | 9 | `recovery_code()` is `Some` for 8 of 13 after §3.9; five code-less variants take `media_backend_unclassified`; `from_media_error` becomes total (rule 12) |
...
16. `pending_observations` (`app.rs:188`) becomes `pub(crate)`. It is the sink's replacement seam and every migrated site outside `app.rs` writes to it; probe-1b P5 measured that this is one line and that no other plumbing is needed.
...
fn verify_claimed_outcome(incident: &Incident, document: &Document,
    ...
    match (incident.code, incident.subject, incident.evidence.probed()) {
        ...
        (IncidentCode::SourceColor(_), IncidentSubject::Asset(asset_id), Some(probed)) =>
            verify_source_colour_outcome(incident, document, outcome, asset_id, probed),
...
```
   ...
   The three refusal codes IN1 added — `incident_not_found`, `incident_not_applied`, `incident_not_reverted` (`server.rs:16415`) — are unchanged, and Part B adds none. `incident_error` (`server.rs:16415`) is unchanged.
...
| --- | --- | --- |
| `crates/kinewright-agent/src/export_queue.rs:412` | the `ExportQueueError::LutStoreRootInvalid { reason }` builder | `export_queue.rs` `cc4_export_separates_a_refused_store_root_from_an_unsaved_project` |
...
| `crates/kinewright-agent/tests/mcp_server.rs:10346` | `in1_the_agent_surface_grows_by_two_capabilities_and_no_tool` | the quad (`:10400`) plus both size constants; its message reads "sixteenth" at `:10401` and becomes **"seventeenth"** |
...
8. IN1 §6.6 rule 25 and §9 clause 13 say "both pin sites"; there are **three**. Both take erratum **R3** at promotion, and each pin site's doc comment gains one sentence naming IN1b and saying why the quad did not move: the two capabilities were already registry-only and `served_tools()` filters by `COMPACT_TOOL_NAMES` (`server.rs:726-730`), which Part B does not touch.
...
11. `in1b_every_subject_variant_labels_and_serialises_in_its_declared_shape` — the seven `label()` strings and the three-shape wire union of §3.11 rule 42, `Chain` included in both its forms.
12. `in1b_the_part_a_wire_body_is_byte_identical` — IN1 §6.2 rule 10's literal, unmodified (`incident.rs:1391` re-run), at 819 B.
13. `in1b_a_project_and_a_chain_subject_incident_serialise_to_their_pinned_bodies` — the second and third wire literals of §3.11 rule 42.
...
16. `in1b_<label>_produces_its_declared_code_class_and_severity` for each of the 17 labels of §2.1, each driving the label's own representative site through `pending_observations` into the log and asserting the code, subject, class and severity Appendix B declares. `"Timeline"` is one of the 17 and is driven through `InspectorEdits::incident_code()` (§5.4 rule 27). Every one of the 17 is writable: after §3.3 every subject Appendix B names is constructible, and after §3.3 rule 20 the `Operations` and `Agent branch` rows have a declared subject to assert rather than a disjunction.
...
33. `in1b_every_code_fits_the_measured_ceiling` — the rewritten `in1_the_fixture_incident_is_pinned_and_every_code_fits_the_ceiling` (`server.rs:~26140`), quantified over **67** codes × **seven** subject shapes, with the fixture pinned at 819 B and the ceiling at 2 048 B.
...
| 5 | All **seven** `IncidentSubject` variants label and serialise in their declared shapes, including `Chain`'s nested union and the three bare strings | §3.3, §3.11 rule 42 | item 11 | six of the seven do not exist (`incident.rs:267` declares `Asset` alone) |
...
| 14 | The three `strip_prefix("media backend error: ")` parsers are gone and each reads `MediaError::recovery_code()`, `room_tone_store_error_result` included | §6.2 rule 4 | items 30–31 | three parsers exist and `room_tone_store_error_result` is untested |
| 15 | `MediaError::recovery_code()` is `Some` for **8 of 13** variants and both matte codes survive the `From` impls typed | §3.9 rule 36 | item 15 | it is `Some` for 6 of 11 (`media.rs:1826`) and both flatten into `Backend(String)` |
| 16 | **The token budget, two assertions that can fail**: `assert_eq!(serde_json::to_vec(&incident).len(), 819)` for the named fixture incident, and `<= 2_048` for all **67** codes across all **seven** subject shapes | §3.11 rule 43 | item 33 | the loop quantifies over 12 codes and one subject (`server.rs:26169`), and the ceiling constant is 1 024, which the measured worst of 1 354 B exceeds |
...
- **P1 — the build gates.** `cargo fmt --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, and **`cargo check -p kinewright-app`** before **each** commit. Every cargo command runs after `source ./scripts/setup-ffmpeg.sh`. P1 is the reason §5.3 rule 21's shim exists.
...
| **B** | media | **No file is edited.** `src/lut_store.rs:135` and `src/room_tone_store.rs:188` are **read** by cut item 2's typed path; `src/in1_fixtures.rs:67` is **read** and its assertions stay green (§9 regression R-B). Probe-2b T3 confirmed no `kinewright-media` file breaks on the token. The R20–R39 erratum range is reserved and expected to be unused. |
...
| 24 | `app.rs:1580` | Operations | `operation_* (family)` | *(from the operation)* | `OpError` | `Explain` | `Blocks` |
| 25 | `app.rs:1585` | Operations | `edit_plan_rejected \| operation_*` | *(from the batch)* | `OpError` | `Explain` | `Blocks` |
| 26 | `app.rs:1598` | Operations | `edit_revision_conflict` | `Project` | `Revision` | `Explain` | `Blocks` |
| 27 | `app.rs:1637` | Media | `MediaError::recovery_code()` (§3.2) | `Asset` | `SourceColor \| MediaError` | `AutoApply \| Explain` | `Blocks` |
...
| 96 | `media_workflow.rs:1886` | Look | `look_unclassified` | `Asset` | `Plain` | `Explain` | `Blocks` |
...
| 99 | `media_workflow.rs:1941` | Look | `look_unclassified` \[cut 2 → `lut_store_*`\] | `Asset` | `MediaError` | `Explain` | `Blocks` |
| 100 | `media_workflow.rs:1953` | Look | `look_unclassified` | `Asset` | `Plain` | `Explain` | `Blocks` |
| 101 | `media_workflow.rs:1966` | Look | `look_unclassified` | `Asset` | `Plain` | `Explain` | `Blocks` |
| 102 | `media_workflow.rs:1974` | Look | `operation_internal` | `Asset` | `Plain` | `Explain` | `Blocks` |
| 103 | `media_workflow.rs:1982` | Look | `look_incomplete` | `Asset` | `Plain` | `Explain` | `Degrades` |
| 104 | `media_workflow.rs:2006` | Look | `look_unclassified` \[cut 2 → `lut_store_*`\] | `Asset` | `MediaError` | `Explain` | `Blocks` |
...
| 117 | `timeline_ui.rs:632` | Operations | `operations_unclassified` | `Clip` | `Plain` | `Explain` | `Blocks` |
| 118 | `timeline_ui.rs:652` | Operations | `operations_unclassified` | `Clip` | `Plain` | `Explain` | `Blocks` |
| 119 | `timeline_ui.rs:665` | Operations | `operations_unclassified` | `Clip` | `Plain` | `Explain` | `Blocks` |
| 120 | `timeline_ui.rs:701` | Operations | `operations_unclassified` | `Clip` | `Plain` | `Explain` | `Blocks` |
| 121 | `timeline_ui.rs:713` | Operations | `operations_unclassified` | `Clip` | `Plain` | `Explain` | `Blocks` |
...
| 128 | `transcript_edit.rs:95` | Transcript edit | `transcript_edit_unclassified` | `Clip` | `Plain` | `Explain` | `Blocks` |
| 129 | `transcript_edit.rs:108` | Transcript edit | `transcript_edit_unclassified` | `Clip` | `Plain` | `Explain` | `Blocks` |
