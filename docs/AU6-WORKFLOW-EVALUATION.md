...
**Review finding (outside AU6 erratum ranges).** `au3_an_export_that_does_not_normalize_is_byte_identical` (`crates/kinewright-media/tests/au3_fixtures.rs`) fails on this Linux / FFmpeg 8 VM on both **`origin/main` (`888fe38`)** and this branch, same assertion: the two un-normalized exports are not byte-identical (`assert_eq!(off, off_again)`, cargo `finished in 20.51s` on main). `src/export.rs`, `src/engine.rs` and `tests/au3_fixtures.rs` are identical to main. AU6 does not change AU3 product code. Windows CI on `888fe38` (run 34619576334) is green. This is not an R20–R39 erratum — those files are AU6-only — and it is not a Part A closer.
...
> **5 of 5 scenarios complete by the person to the identical canonical document; 6 of 6 tasks by the scripted agent; the model lane is pending the real-harness run.**

Published in `au6_manifest.json` under `scorecard.person_agent_parity` and asserted equal to the count of `Au6PersonPath::Expressible` entries. **If §6.2 slips**, `au6_spec(Interview).person_path` becomes `NotApplicable { reason }` naming `MixerChain`, and the sentence becomes *"4 of 5 … with the interview's ducking ride reachable only through the bus curve or a sidechain node"* — a checked fact, and one the probe has already shown produces a bit-identical programme.
...
5. **The Windows lines.** The first green Windows CI run's printed `AU6_*` margin lines are transcribed into Appendix A row W with the run id — recorded, never gated (N1 Q10).
...
- **Wall clock.** Measured: `mix_levels` **8 829 ms** over one 12 s document and **1 402 ms** over one 2 s turn; `mix_window_levels` **266–990 ms** whole-programme; `audio_qc` **1 626–2 391 ms**; `audio_repair` **465 / 3 900 ms** bare / three-node; a normalized 8 s 320×180 encode **13 898 ms** plus **941 ms** verification (probe-1 §9). Projected: media lane **≈ 90 s** with (d)'s leg cut (**≈ 105 s** with it), agent lane **≈ 60 s** (5 / 9 / 15 / 5 / **24** for `a5a`'s 12 s encode / **2** for `a5b`'s queue-and-cancel, B2/B3), so AU6 adds **≈ 150 s** to `cargo test --workspace` on top of the existing ≈ 100 s audio floor (media lib 51.8 s, `au3_fixtures` 49.0 s; N0.3(c), N0.7(f)). `ci.yml` has no `timeout-minutes`, so the failure mode is slow rather than red; `AU6_MEDIA_LANE_BUDGET_SECONDS = 180` and `AU6_AGENT_LANE_BUDGET_SECONDS = 120` are recorded, never gated. **The call pattern is normative because it is the difference between 105 s and 300 s** (§10.3): one `mix_levels` per authored turn, one whole-programme `mix_window_levels` render per point, `audio_qc` for LRA. Fallback: the cut order (§12).
