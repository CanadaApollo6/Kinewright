...
**Still Linux-only, and named as such:** `CC7_MEASURED_DELIVERY_EIGHT` / `_TEN` are the worst-per-term *summary* constants in `kinewright-core`. Nothing compares them to a live measurement — the delivery gate measures against the manifest, and the manifest-to-code assertion is a consistency check that holds on both systems — so they did not need splitting to make Windows pass. They remain Linux's summary of Linux's column, which is a documentation gap rather than an ungated claim, and splitting them is the one piece of this decision left.
...
- **Splitting `CC7_MEASURED_DELIVERY_EIGHT` / `_TEN`.** *Decided and mostly discharged.* Riel ruled on 2026-09-12 that per-OS constants are fine when each system's numbers are measured and optimized on that system; AU3 §0 **E59** carries the rule, CC6 §6.3 and §10.6 follow it, and CC7's measured column, provenance and neutral-chroma expectation are split accordingly (§0.3 review finding 5). What is left is the two **summary** constants in `kinewright-core`: they hold Linux's worst-per-term figures, nothing compares them to a live measurement, and so they did not have to move for Windows to pass. Splitting them means a `cfg(target_os)` pair with the Windows worst-per-term computed from the column now in the manifest, and a per-OS `terms` measured column beside it — an hour, and it closes the last place a Linux number stands in for both.
- **The `AU6_MEASURED_*` column is Linux's too.** The same gap, one slice over: AU6's measured constants were all taken on Linux, and the AU6 lanes pass on Windows without them being split, because each is compared to a budget rather than to a second system's figure. Now that Appendix A row W records a Windows pass, a `--nocapture` Windows run (§13 item 5) would show which AU6 rows actually differ — on CC7's evidence expect few — and each that does earns its own column under E59 rather than a widened budget.
