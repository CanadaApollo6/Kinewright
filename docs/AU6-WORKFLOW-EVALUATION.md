...
- **R20.** The live media-lane wall on this Linux debug lane at `049eb90` is **437 s** (`cargo test -p kinewright-media --lib au6_ -- --test-threads=1`, 59 passed, cargo `finished in 437.01s`). A1's **≈ 90 s** was a probe projection, not a live fixture-suite measurement. `AU6_MEDIA_LANE_BUDGET_SECONDS` stays **180** — recorded, never gated, never widened (S16). The per-group `breakdown` keys remain probe-1's projection; this run did not emit per-test times, so they are not re-derived. *Was:* `performance.media_lane.measured_seconds: 90`.
...
- **R40.** The live agent-lane wall at `080243e` is **134 s** (`cargo test -p kinewright-agent --test mcp_server au6_ -- --test-threads=1`, 8 passed, 134.24 s). `AU6_AGENT_LANE_BUDGET_SECONDS` stays **120** — recorded, never gated, never widened (S16). The per-script breakdown remains probe-1's 60 s projection (N14). *Was:* measured ≈ 60 s against budget 120.
...
**Review finding 1 (outside AU6 erratum ranges) — AU3's non-normalize export identity, resolved.** `au3_an_export_that_does_not_normalize_is_byte_identical` (`crates/kinewright-media/tests/au3_fixtures.rs`) failed on this Linux / FFmpeg 8 VM on both `origin/main` (`888fe38`) and this branch, same assertion: the two un-normalized exports were not byte-identical (`assert_eq!(off, off_again)`, cargo `finished in 20.51s` on main). `src/export.rs`, `src/engine.rs` and `tests/au3_fixtures.rs` were identical to main and AU6 changed no AU3 product code. **It is fixed on `main` by PR #6 (`07dd22b`, `video_options.set("threads", "1")` on the delivery libx264 path), merged at `d182d45`** — separately, as a review finding rather than an AU6 erratum, because those files are not AU6's. The deferral this note used to carry is discharged: Linux CI ran the full `cargo test --workspace` **clean at `0b2290b`** (run 34633080767), which is the contract's actual gate, so no workspace-closer is outstanding on Linux.

**Review finding 2 (outside AU6 erratum ranges) — Windows CI has never run an AU6 test, and AU6 has no Windows evidence.** `cargo test --workspace` on `windows-latest` at `0b2290b` runs exactly **one** test binary — `kinewright_agent`'s lib unittests — which fails **4 / 491** and aborts cargo before `kinewright-core`, the 59 AU6 media fixtures, the app tests or `tests/mcp_server.rs` run. The job nevertheless reported `conclusion=success`, because the `pwsh` block ran `setup-ffmpeg.ps1; cargo build; cargo test; cargo clippy` without checking `$LASTEXITCODE`, so the step's status was clippy's. Two consequences, both recorded rather than papered over:

1. **The step now fails on the first failing command** (`.github/workflows/ci.yml`, this branch), so Windows CI reports what it finds. It will be **red on this PR** until the four failures below are fixed; that is the honest state, not a regression this slice introduced.
2. **Every "Windows CI green" claim about AU6 is withdrawn.** §13 checklist item 5 and Appendix A row W plan to transcribe "the first green Windows CI run's printed `AU6_*` margin lines"; no such run exists, and none has existed for as long as these four failures have been in the tree, which also puts the AU2–AU5 "Windows CI green" statements in doubt. Scope's "both CI operating systems" is **unproven for AU6 on Windows**.

The four failures are pre-existing and unrelated to AU6 — identical on `main`'s run 34637198357 — and are a named follow-up, not AU6 work (§13):

| Test | Cause |
| --- | --- |
| `export_queue::tests::au3_a_normalizing_job_runs_the_profile_target_and_records_its_report` | `\\?\C:\Users\runneradmin\…` against `C:\Users\RUNNER~1\…`, an 8.3 short-path canonicalization |
| `export_queue::tests::cc6_a_verified_export_publishes_its_decoded_comparison_on_the_record` | the same path canonicalization |
| `export_queue::tests::a_source_swapped_mid_encode_fails_the_job_instead_of_completing_it` | the same, inside the quarantine message |
| `color_status::tests::circular_median_agrees_with_brute_force_and_survives_two_million_samples` | a timing assertion, 2.36 s on the runner |
...
- **The four pre-existing Windows `cargo test --workspace` failures.** Three `export_queue::tests` cases compare `\\?\C:\Users\runneradmin\…` against the runner's 8.3 short path `C:\Users\RUNNER~1\…`, and `color_status::tests::circular_median_agrees_with_brute_force_and_survives_two_million_samples` asserts a wall clock that reads 2.36 s on `windows-latest`. All four are on `main` (run 34637198357) and none is AU6's; the first three want one canonicalization helper applied on both sides of the comparison, the fourth wants a bound that is not a timing assertion. Half a day, in `kinewright-agent`. **Until they are fixed the Windows job is red**, because this branch makes the step fail on a failing command (§0.3 review finding 2) instead of reporting clippy's status. Every AU6 Windows figure — §13 item 5, Appendix A row W, §14's cross-OS FFmpeg risk — is blocked behind them.
...
5. **The Windows lines.** The first green Windows CI run's printed `AU6_*` margin lines are transcribed into Appendix A row W with the run id — recorded, never gated (N1 Q10). **Blocked:** no Windows run has ever executed an AU6 test, and none can until the four pre-existing failures above are fixed (§0.3 review finding 2). The row stays empty and no AU6 claim rests on it.
