...
- **R24. The two eval `include_str!`s are removed until Part B needs them.** `AU6_TEST_SOURCES` embedded `crates/kinewright-agent/src/eval.rs` (12 064 lines) and `src/bin/kinewright-eval.rs` (7 631) into the media test binary so the inventory could scan them for `AU6_EVAL_TESTS` — an array §11.4 declares `[&str; 0]` for Part A. Both directions of the scan were **vacuous** over them: no name to look for, and no `au6_` test in either file to find. The cost was **19 695 lines** of unrelated source as `&'static str` plus a rebuild of media's tests on every edit to either file. `AU6_TEST_SOURCES` is **9** entries and `au6_inventory_groups` **4** rows; the forbidden-helper and `cfg(target_os` scans never covered those two files anyway (they run over eight named paths). The saving cannot become a missing gate: `au6_declared_test_names_exist_in_their_source_files` now asserts `AU6_EVAL_TESTS.is_empty()` with the message that names both files and both counts to restore, so the first Part B eval test fails it and says what to put back. *Was:* eleven sources and five groups. §11.4's counts are read through this erratum.
- **R25. §2.8's twelve neighbour budgets are pinned to their owners.** They were twelve `const`s in `tests/au6_core.rs` with their owners named only in a doc comment and **nothing comparing them**: `kinewright-core` cannot see `kinewright-media/tests/au3_fixtures.rs`. If AU3 moved `FIXTURE_LOUDNESS_BUDGET_HUNDREDTHS`, the 147-line distinctness test went on passing while no longer testing what it claimed — the one place in the slice where the evidence could rot in silence, and the asymmetry is sharp because the six `AU6_*_RESTATED` constants *are* checked against their owners. The table moves to `au6_scenarios` as `AU6_NEIGHBOUR_BUDGETS: [Au6Neighbour; 12]`, each entry carrying its owner's constant name **and file**, which also removes the duplicate twelve-tuple the test built beside the twelve consts. The media lane — which can see both owner files — gains **`au6_neighbour_budgets_agree_with_their_owners`**: it reads the literal each owner declares and asserts the restatement equals it, with a `assert_ne!` on one code off so the pin cannot pass by matching nothing. **Ten of twelve are pinned**; `LoudnessTarget.tolerance_lu_hundredths` is compared **by value** in core where the struct is reachable; and `DECLICK_ERROR_DROP_BUDGET_TENTH_DB` is a **function-local** `const` in an eleven-thousand-line file (`media/src/audio.rs:11004`), reachable by neither name nor a cheap text pin, so it is the one restatement still trusted — asserted as such by name, so the fixture claims ten rather than implying twelve. `AU6_MEDIA_TESTS` is **60** and §11.2 gains the entry. Cost: two `include_str!`s totalling 1 654 lines, two orders of magnitude below what R24 removed.
- **R26. `test-util` now gates what the tree always claimed it gated.** `au6_sources`' module doc and media's `lib.rs` both said the generators sit "behind the `test-util` feature no default build enables". That was false: `crates/kinewright-agent/Cargo.toml` enabled `kinewright-media/test-util` as a **normal** dependency and `kinewright-app` depends on the agent, so `cargo build --release -p kinewright-app` turned it on. And `kinewright-core` had **no feature at all**, so `au6_scenarios` (2 555 lines) and `cc7_scenarios` (2 162) compiled into every release build unconditionally. Fixed: `kinewright-core` gains `test-util`, both scenario modules and their re-exports sit behind `#[cfg(any(test, feature = "test-util"))]`, media's `test-util` chains to core's, and the agent's normal dependency drops the feature. **7 784 lines leave the shipping binary** (`au6_scenarios` 2 555 + `au6_sources` 1 049 + `cc7_scenarios` 2 162 + `cc7_sources` 1 550 + `test_support` 468); `cargo tree -p kinewright-app -e features,normal` now contains **no** `test-util` edge, and `cargo build --release -p kinewright-app` is green. *Erratum (2026-09-15):* Part B (`3469924`) then imported `kinewright_core::au6_scenarios` at module scope in `crates/kinewright-agent/src/eval.rs` for the audio evidence block, so the release build of the app failed again while every workspace-wide command stayed green through feature unification. `pub mod eval` is now behind the agent's `eval-harness` feature; the shipping binary carries neither the fixture data nor the harness that consumes it.
  **Not `required-features`, and the reason matters.** The `kinewright-eval` binary imports `kinewright_media::cc7_sources`, `test_support` and `kinewright_core::cc7_scenarios` at module scope, so it genuinely needs the gates at build time. Gating the target on a non-default feature would have made `cargo build --workspace`, `cargo test --workspace` **and** `cargo clippy --workspace --all-targets` skip it in silence — 7 631 lines unlinted and **38 tests** unrun, including `published_v6_manifest_tracks_the_color_workflow_suite`. Instead the agent carries `default = ["eval-harness"]` and **`kinewright-app` takes the agent with `default-features = false`**: every workspace command still builds, runs and lints the harness with no extra flag, and the shipping binary links an agent library with no fixture data in it. Each crate's `[dev-dependencies]` re-enable the gates for its own tests, which resolver 3 keeps out of normal builds.
...
**Eval (P/Q), `src/eval.rs` / `src/bin/kinewright-eval.rs` / `benchmarks/auto-edit/v7/`, 2026-09-14:**

- **R80. R24 wins over §12.3 step 13.** Step 13 says `AU6_TEST_SOURCES` is unchanged because the two eval `include_str!`s were already in Part A. R24 removed them: both directions of the scan were vacuous while `AU6_EVAL_TESTS` was `[&str; 0]`, at a cost of 19 695 lines of unrelated source in the media test binary. Part B restores `crates/kinewright-agent/src/eval.rs` and `src/bin/kinewright-eval.rs` (9 → **11** sources, 4 → **5** inventory groups) so the both-direction scan can see `AU6_EVAL_TESTS: [&str; 12]`. The `AU6_EVAL_TESTS.is_empty()` tripwire is replaced by CC7's `named.retain` over `AU6_EXPLICIT_TEST_NAMES`. *Was:* step 13's "unchanged"; R24's empty array.
- **R81. Measured Part B insertion count.** `git diff --stat origin/main` on this commit: **4 250** insertions (3 000 in the twelve tracked files plus 1 250 in `benchmarks/auto-edit/v7/`) across `eval.rs` (1 853), `kinewright-eval.rs` (1 014), the inventory reopen, the §12.1 docs, and the published v7 package. Replaces §12's ~3 200 + 400 + 700 Part B share of the 11 000–15 000 whole-slice estimate.
- **R82.** `au6_v6_deliverables_are_byte_identical_without_normalization` cannot `assert_eq!` two `ExportSettings`: `ExportCancellation` compares by `Arc` identity. The test compares `serde_json::to_value` instead; cancellation is `#[serde(skip)]`.
- **R83.** `au6_leaked_needle` takes the **longest** matching needle so `detector` does not shadow `detector_threshold_tenth_db`. The self-check haystack is `>>>{needle}<<<`, not `blind-{needle}-payload`: a numeric needle such as `400` matches inside `-400`.
- **R84.** §12.1(9c)'s slice-list comment at `server.rs:26649` is gone (line numbers drifted; no remaining "AU5 §4.3/A18" comment). The ledger assertion at `tests/mcp_server.rs:2449` now names Part B beside `:2485`. No comment was invented in `server.rs`.
...
2. **Every "Windows CI green" claim about AU6 made before that run is withdrawn — and then earned.** For as long as those four failures were in the tree no Windows run could reach an AU6 test, which also puts the AU2–AU5 "Windows CI green" statements in doubt. With them fixed, run **34661190014** ran all **94** AU6 tests on `windows-latest` and passed them (review finding 5), so scope's "both CI operating systems" is **discharged for the AU6 lanes** — measured, not assumed. What the earlier claims lacked was not the result but the evidence.
...
| Test | What Windows reports | Class |
| --- | --- | --- |
| `lut_store::tests::store_root_is_the_project_stem_plus_the_asset_suffix` | the fixture hard-codes a POSIX path, so Windows resolves `/home/riel/edits/…` to `D:\home\riel\edits\…` | test hygiene |
| `loudness::tests::the_true_peak_detector_note_names_the_au3_meter` | an `include_str!` source-text pin whose needle spans a newline; the Windows checkout is CRLF (the same class as review finding 4) | test hygiene |
| `derived_cache::tests::content_hashes_do_not_trust_same_size_same_mtime_replacement` | `PermissionDenied` at `derived_cache.rs:463` — the test rewrites a file Windows still holds open | test hygiene |
| `export::tests::delivery_filter_converts_sixteen_bit_full_range_rgb_to_limited_yuv420p` | neutral 8-bit chroma is `{128}` on Linux and `{128, 129}` on Windows: the two FFmpeg packages round swscale's full→limited conversion differently | **measurement, policy** |
| `cc7_fixtures::cc7_every_scenario_verifies_at_eight_bits` | a declared **18 677** against a measured **18 688** | **measurement, policy** |
| `cc7_fixtures::cc7_every_scenario_verifies_at_ten_bits` | a declared **181 593** against a measured **181 569** | **measurement, policy** |
| `cc7_fixtures::cc7_manifest_declares_every_required_fixture_and_constant` | `"linux"` against `"windows"` — **not a measurement at all**: the manifest records `measurement.os` (`cc7_manifest.json:1697`) and the test asserts it equals the operating system it is running on, so this fixture **cannot pass on Windows by construction**, whatever the encoder produces | **measurement, policy** |

**All seven are pre-existing, and that is checked rather than asserted.** This branch's **only** edit to `cc7_manifest.json` is scenario f's ten-bit `luma_mean_code_millionths` (281 → 274) and `rgb_mean_code_millionths` (77 446 → 77 447). Windows fails on **18 677** and **181 593**, both of which are in the manifest and appear **nowhere in this branch's diff**, and on `measurement.os`, which this branch does not touch either. `derived_cache.rs`, `export.rs`, `loudness.rs` and `lut_store.rs` are not in this branch's diff against `main` at all. So the CC7 re-baseline of review finding 3 is not implicated in any of them; they were unreachable because the Windows job aborted in `kinewright-agent` and then reported success.

**Resolved 2026-09-12 by Riel's ruling, and the split is smaller than expected.** Per-OS constants are allowed when each system's numbers are measured and optimized on that system rather than one budget being loosened to span both (AU3 §0 **E59**, the normative rule; CC6 §6.3 and AU6 §10.6 amended to follow it). Implemented: CC7's measured column and its measurement provenance are keyed by operating system, a budget stays single and shared, and a system with no column fails loudly with everything it measured, so one CI run publishes that platform's whole column instead of surrendering one number per run. The Windows column was harvested that way from run **34693346093** and written in with its own margins.

**What the two builds actually disagree about: 16 of 60 delivery rows.** The other **44 measured identically** and stay scalars, which under the rule means "measured on every supported system and found equal" rather than "Linux's number, assumed". The 16 are every scenario's eight-bit `rgb_mean_code_millionths` and `psnr_db_hundredths`, three eight-bit `luma_mean_code_millionths`, and one ten-bit `rgb_mean` — and in **every one of them Windows decodes closer to the source**: lower RGB mean difference, higher PSNR. The neutral-chroma split is one plane, not two: Windows' scaler returns the exact Cb code where Linux's dithers across two, and **Cr is the same pair on both**, measured to agree rather than assumed to. One row, `e`'s eight-bit `luma_mean`, sits at **1.06×** on both systems — inside its budget, under the 2× bar, identical on each; that is pre-existing, is why CC7 declares it `Cc7BudgetKind::RecordedMargin`, and is not something the per-OS split introduced.

**The summary constants are split too, and the ten-bit one deliberately is not.** `CC7_MEASURED_DELIVERY_EIGHT` is the worst row per term of a column, so it follows the column: Windows' RGB mean is **787 598** against Linux's 855 810 and its PSNR floor **4 070** against 4 059 — both in Windows' favour — with the other three terms measured there and found equal rather than copied. The shared 1 750 000 budget therefore reads **2.04× on Linux and 2.22× on Windows**, which is what optimizing per system instead of widening one constant looks like. `CC7_MEASURED_DELIVERY_TEN` stays a single value because its worst row per term is **identical** on both systems — only scenario (b)'s ten-bit RGB mean differs between the builds at all (181 593 against 181 569) and it is not the worst — so one value is the honest record, measured on both and found equal.

**Both operating systems are green.** CI run **34695157900** at `cb3df47` is the first run in this repository's history with **`Windows workspace: success` and `Linux workspace: success`** under a gate that actually fails on a failing test, and run **34696625455** at `38a5e66` holds it with the summary constants split. Nothing in the per-OS split was chosen to make a red build green: every Windows value was measured on `windows-latest` and written with its own margin, and the 44 rows that agree were left as single values because they were measured to agree.

**Why the four were not this slice's to fix before that ruling.** They are exactly the risk §14 names: Windows CI provisions **a different FFmpeg package** — `System233/ffmpeg-msvc-prebuilt ffmpeg-8.0.1-r3` against Linux's `mifi/ffmpeg-builds 8.0-1` — and no AU3, CC6, CC7 or AU6 number has ever been measured with it, so "the first Windows run **is** the measurement". Every available response is currently forbidden by a contract: AU3 §0 E55, CC6 §6.3 and AU6 §10.6 all say a per-OS figure earns **a per-OS note on the constant's doc comment, never a per-OS constant**, and §4.1 note 1 forbids inventing a tolerance to absorb it. A single manifest cannot hold two platforms' measured columns, and the `measurement.os` row makes that structural rather than arithmetic: CC7's manifest self-test asserts *one* operating system, so CC7's own exit gate has never been satisfiable on both. The three real options — provision one FFmpeg build on both operating systems, publish a per-OS measured column and amend the three contracts that forbid it, or scope the encoded fixtures to Linux and say so in each contract's exit gate — are a cross-slice decision, not an AU6 erratum. §13 carries it.

**AU6 itself is green on Windows, and that is now measured rather than hoped for.** Run **34661190014** at `1f09c18` ran **94 AU6 tests on `windows-latest` and passed all 94** — core **13/13**, agent **8/8**, app **14/14**, media **59/59**, zero failures — which is the first time any AU6 test has executed on that operating system. Scope's "every objective claim discharged by an ordinary `cargo test` on **both CI operating systems**" is therefore **discharged for the AU6 lanes**: every Windows margin cleared its budget, the `f64` seam still read exactly zero, and the 31-band profile pin still held exactly, on a different FFmpeg package from the one they were measured with.

**What is still open is narrower than it was.** The Windows *job* remains red on the seven pre-existing `kinewright-media` failures above, none of them AU6's. And the **printed** `AU6_*` margin figures are still not transcribed, because `cargo test` captures a passing test's stdout: the gates passing tells us every margin cleared, not what each one measured. §13 item 5 and Appendix A row W therefore stay empty for the *figures* while recording that the lanes pass — a distinction this document keeps rather than rounding either way.

**Review finding 6 (outside AU6 erratum ranges) — two `kinewright-app` recovery tests are a pre-existing flake on one 2-second deadline, and this branch is only why they are visible.** Run **34699358343** at `98a6441` failed Windows on `recovery::tests::recorder_subscribes_to_human_and_agent_core_events` and `recovery::tests::stale_journal_from_crashed_child_is_detected_and_replayed`. That commit changed `au6_manifest.json` and one paragraph of this file; **`crates/kinewright-app/src/recovery.rs` has no commit and an empty diff against `main` on this branch.**

**They are one failure, not two.** Both tests call `recorder.flush()`, which waits on a channel for `CORE_RESPONSE_TIMEOUT` — **2 seconds** (`recovery.rs:44`). The first reports it directly: *"recovery recorder did not flush in time: timed out waiting on receive operation"*. The second spawns the test binary again as a **child** (`crash_child_process`), and that child also calls `recorder.flush().unwrap()` before `std::process::exit(73)`; when the child misses the same deadline it panics, exits **101**, and the parent's `assert_eq!(status.code(), Some(73))` reports `Some(101)` against `Some(73)`. The child's `stderr` is `Stdio::null()`, so the message that would have said so is discarded — which is the whole reason the second failure reads as an unexplained exit code.

**Why it flakes under a full suite, and why now.** The child is a fifth process on a four-core runner while the parent binary is already running its 497 tests four-way parallel, so the 2-second deadline is likeliest to be missed exactly when the suite is busiest. Before this branch the Windows job **never reached the app test binary at all** — it aborted in `kinewright-agent`'s lib binary and reported success, and `main`'s run 34637198357 and this branch's 34644673292 both contain **zero** `recovery::tests::` lines. These tests have therefore run on Windows four times in the repository's history: green at `cb3df47` (run 34695157900), green at `38a5e66` (run 34696625455), **red at `98a6441`** (run 34699358343), and green again at `81871a6` (run **34700735921**, both jobs success) — where neither test nor `recovery.rs` had changed between the red run and the green one after it. Same code, three passes and one failure: a flake, on the record rather than argued.

**Not fixed here, deliberately.** Nothing on this branch caused it and `recovery.rs` is not AU6's file, so churning a product recovery path to chase a timing flake would be the wrong trade. §13 carries it with the two changes whoever takes it should make: give the child's failure a voice (keep its `stderr`, or have it write its own error to the journal directory so the parent can report the real message instead of an exit code), and make the flush deadline proportionate to a loaded CI box rather than a flat two seconds — or wait on an acknowledgement with no deadline at all, since the test already bounds the work.

**Review finding 7 (outside AU6 erratum ranges) — three §13 agent/ledger nits, paid.** `remove_audio_bus` `destructiveHint` is `true`; `QueueExportArgs` denies unknown fields; the `-6000..=3600` gain guard is media's published constants. Registry **1 540 292 / 1 397 185 / 120 458**; served quad unchanged. Typed envelopes, `queue_export` raster, and `plan_track_mix` stay deferred.
...
2. **60 Hz drop, with per-harmonic selectivity.** *Measures:* the same report's `hum_60_excess_db_hundredths` (`audio_repair.rs:117`) and the four-entry `hum_60_harmonic_excess_db_hundredths` (`:125`). *Passes:* `excess(before) − excess(after) ≥ AU6_HUM_DROP_MIN_DB_HUNDREDTHS = 2_400`; measured **4 970** (9 163 → 4 193 on the planner chain), margin **2.07×**; **and each of the FIRST THREE harmonics** drops by ≥ `AU6_HUM_HARMONIC_DROP_MIN_DB_HUNDREDTHS = 500` — measured **1 096 / 1 706 / 2 272** at 60 / 120 / 180 Hz, worst margin **2.19×**.
   ...
   **No AU6 gate asserts the absence of `mains_hum_present` after repair, and no AU6 gate reads the 50 Hz term (A7/S7).** The 60 Hz cascade's sixth-octave shoulders used to reshape 50/100/150 Hz enough that `hum_50_excess_db_hundredths` rose from **0 to 829** (planner) or **1 072** (media chain) and a *second* `mains_hum_present` appeared after repair. **Paid in §13:** the shoulder reference now excludes notched neighbours; the AU6 60 Hz drop and per-harmonic gates are unchanged.
...
3. **The two deliveries separate by the target difference.** *Passes:* `|(streaming − source_master) − AU6_TARGET_SEPARATION_LU_HUNDREDTHS (900)| ≤ AU6_TARGET_SEPARATION_TOLERANCE_LU_HUNDREDTHS = 25`; measured **900** at 8 s (**901** at 12 s), so the term is **0–1**, margin **25×**. **Cut 2026-09-14** (§12 / §13). *Fails:* **`au6_e_two_exports_at_one_profile_do_not_separate`**, measured **0** separation.
...
### 5.4 M36 bookkeeping — AU6 itself does not move the surface; the §13 follow-up moves registry bytes only

- `COMPACT_TOOL_NAMES` stays at **7** (`crates/kinewright-agent/src/runtime.rs:17-25`); `served_surface_is_small_and_keeps_the_internal_registry_discoverable` (`crates/kinewright-agent/src/server.rs`) still reports the served quad **7 / 5 660 / 3 510 / 998** and the registry triple **1 540 292 / 1 397 185 / 120 458**.
...
>
> AU6 §13's agent/ledger follow-up adds no capability either: it flips `remove_audio_bus`'s `destructiveHint` and closes `QueueExportArgs`. The registry moves to 1 540 292 / 1 397 185 / 120 458. The served quad does not move for the fifteenth consecutive measurement.
...
**`published_v7_manifest_tracks_the_audio_workflow_suite`** asserts, both directions: task ids; `machine_assertions` counts; every `budget` field; every `prompt`; every `delivery.*` **including `normalize_to_profile_target`**; every `human_question` against `AU6_QUESTIONS`; and each ground-truth document against `au6_canonical_operations` applied to its base document. **It also asserts every `machine_assertions` string is non-empty, distinct within its task, and names either an `au6_scenarios` constant or a tool in `capability_tool_names()`** — the weakest check that would have caught v5's `g3` drift. **v1–v6 now run that same string check** (`published_v1`–`published_v6`); g3's published strings track `music-ground-truth-v10` and refuse the leftover v9 aftermath window `1285..1345` (N0.4(d), §13 paid).

### 7.10 CI and the real-harness run

CI is two jobs (`windows`, `linux`), each `cargo fmt --check`, `cargo test --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`, with **no `KINEWRIGHT_EVAL`, no `--ignored`, no `timeout-minutes` and no GPU job**. Both jobs restore `target/` with `Swatinem/rust-cache@v2` after `dtolnay/rust-toolchain` (the default key hashes rustc and every `Cargo.lock` / `Cargo.toml`). Linux also runs `cargo build --workspace` so the `readelf` soname gate can see `kinewright-eval` before tests; Windows skips that extra MSVC compile because `cargo test` already builds the workspace. CI covers the suite's **unit** tests: construction, assertion dispatch, the evidence block's both-direction plumbing test, packaging, the leak test and `published_v7_...`. **Running v7 against a real harness is Riel's action**, as v6's is; `docs/EVALS.md` records "pending real-harness run". **There is no published v6 result to compare against** and AU6 does not pretend otherwise.
...
6. **No `cfg`-conditioned budget or tolerance, ever — but a measured column is per operating system.** **Amended 2026-09-12 (Riel); AU3 §0 E59 is the normative rule and this clause follows it.** A **budget** and a **tolerance** are single, shared and never `cfg`-conditioned, and neither is ever widened to span two systems. A **measured** value is either a scalar — measured on every supported system and found equal — or keyed by operating system, **each value measured on that system and optimized there**, with its own provenance recorded beside it; a system with no column fails loudly rather than passing by omission, and no number is copied across systems. `cfg(target_os)` is permitted for a measured constant on those terms and for nothing else; §11.0.6's scan, which forbids `cfg(target_os` in any AU6 *fixture* file, is unchanged and AU6 still adds none. *Was:* "A Windows figure inside a raw budget but under 2× margin earns a per-OS note in the constant's doc comment, never a per-OS constant" — which let a Linux number stand in for a Windows one it had never been compared against, and which the first working CI gate showed to be false for the encoded lanes (§0.3 review finding 5).
...
30. **`au6_b_the_programme_stays_inside_its_loudness_range`** — key `budgets.podcast.lra`, via `audio_qc`. *Fails:* **`au6_b_the_makeup_less_chain_exceeds_the_loudness_range`**. **Cut 2026-09-14.**
...
43. **`au6_e_the_two_deliveries_separate_by_the_target_difference`** — key `budgets.separation`. *Fails:* **`au6_e_two_exports_at_one_profile_do_not_separate`**. **Cut 2026-09-14.**
...
44b. **`au6_neighbour_budgets_agree_with_their_owners`** — key `transcriptions` (**R25**). Every §2.8 neighbour in `AU6_NEIGHBOUR_BUDGETS` is asserted equal to the value its owner file declares, read out of `tests/au3_fixtures.rs` and `tests/au5_fixtures.rs` by `include_str!`; ten of the twelve are pinned this way, `LoudnessTarget.tolerance_lu_hundredths` is compared by value in core, and `DECLICK_ERROR_DROP_BUDGET_TENTH_DB` is named as the one restatement still trusted because its owner is a function-local `const`. *Fails:* a value one code off the owner's is asserted **not** to be found, so the pin cannot pass by matching nothing.
...
AU6_TEST_SOURCES: [(&str, &str); 9]   (eleven until R24; see below)
...
8. **Performance regeneration and the inventory, last (K).** Re-measure both `performance` lanes on the finished Part A tree in one run and paste them into the manifest; then `AU6_MEDIA_TESTS`, `AU6_CORE_TESTS`, `AU6_AGENT_TESTS`, `AU6_APP_TESTS`, **`AU6_EVAL_TESTS: [&str; 0]`**, `AU6_INVENTORY_TESTS`, `AU6_EVIDENCE_FIXTURES`, `AU6_EXTERNAL_OWNERS`, `AU6_TEST_SOURCES: [(&str, &str); 9]` (eleven at Part B, R24), `AU6_EXPLICIT_TEST_NAMES: [&str; 0]` (filled at step 13), `AU6_FORBIDDEN_HELPERS`, the plain `cfg(target_os` assertion, and items 79–80 (§11.4). `include_str!` binds this file to step 6's and step 7's sources **at compile time**, which is why nothing here can be authored earlier and why the both-direction assertion would fail on any `au6_` test that exists but is not yet declared. *Size ≈ 400.* Then Part A's two review passes per crate (core, media, agent, app), the workspace gate, and the commit.
...
- **`RemoveAudioBus` and bus rename controls.** Half a day (N1 S3(b)): a destructive strip control needs the confirmation shape the timeline's delete already has, a rename needs an inline text edit the mixer has no precedent for, and both need their DESIGN.md sentence. **Not free:** the person path for every AU6 scenario builds buses and never removes one, so the gap costs no scenario. *With it:* the **AU2 `remove_audio_bus` destructive-annotation asymmetry** — **Paid.** `destructiveHint` is now `true` (`crates/kinewright-agent/src/schema.rs`), matching `plan_preview`'s destructive list. Registry serialized −1 B (`"false"` → `"true"`); descriptions and input schemas unchanged; served quad unchanged. The mixer strip/rename controls themselves stay the product gap this bullet named.
...
- **A loudness-matched A/B hold.** **Paid.** A monitor gain stage after the master, not the master fader. Hold matches the live integrated figure to the export-dialog target; release returns the monitor to unity. App-only: no `Operation`, no tool, no ledger byte. This is why §8.0 asked for one listening level.
- **An eyedropper beyond `Learn profile`; an audio QC pane; bus solo/mute; scrub audio.** The QC pane is two days (a surface for `AudioQcReport` the app has no precedent for); bus solo/mute is half a day of app **plus** an `AudioBus` field, i.e. a schema byte, so it is not app-only; scrub audio is engine work. **Not free:** every AU6 scenario completes by hand without them (§6.6).
...
- **The manufactured 50 Hz `mains_hum_present` finding after a 60 Hz cascade (E7, A7). Paid.** The estimator now drops a sixth-octave shoulder that sits within a sixth-octave of the other mains family's harmonics (`hum_shoulder_hertz` in `crates/kinewright-core/src/audio_repair.rs`). When both sides are neighbours the reference is the **max** of the two, so a hole on either side cannot pull it down. Report shape is unchanged — no joint two-family fields. A 60 Hz-only cascade stays under `REPAIR_HUM_EXCESS_HUNDREDTHS` on `hum_50` and raises no 50 Hz `mains_hum_present`; a both-families fixture keeps the real 50 Hz finding. AU6's 60 Hz drop (`AU6_HUM_DROP_MIN_DB_HUNDREDTHS`) and first-three per-harmonic gates (`AU6_HUM_HARMONIC_DROP_MIN_DB_HUNDREDTHS`) stay. *Was:* `hum_50_excess_db_hundredths` rose from **0 to 829** (planner) / **1 072** (media) because the 50 Hz high shoulder at 56.12 Hz sat on the 60 Hz notch.
...
- **The two `kinewright-app` recovery tests' 2-second flush deadline.** A pre-existing flake on `windows-latest`, diagnosed in §0.3 review finding 6: one deadline, two symptoms, and the child process's `stderr` thrown away so the second reports an exit code instead of a reason. Neither test is AU6's and neither was touched by this slice; they are simply the newest thing the working CI gate made visible. Half a day in `kinewright-app`: give the child's failure a voice, and make the deadline proportionate to a loaded runner — or drop it, since the test already bounds the work it waits for. **Not free:** until it is done, a Windows run can go red on a green tree, which is exactly the ambiguity the exit-code fix existed to remove.
- **The `AU6_MEASURED_*` column is Linux's.** CC7's per-OS split is complete and both operating systems are green (§0.3 review finding 5), but AU6's own measured constants were all taken on Linux. The AU6 lanes pass on Windows without them being split, because each is compared to a **budget** rather than to a second system's figure — so this is a bookkeeping gap, not an ungated claim. Now that Appendix A row W records a Windows pass, a `--nocapture` Windows run (§13 item 5) would show which AU6 rows actually differ; on CC7's evidence expect few, since 44 of its 60 rows agreed exactly. Each that does earns its own column under AU3 §0 E59 rather than a widened budget, and the rows that agree earn the stronger statement — measured on both, found equal.
- **The printed `AU6_*` Windows margin figures.** The lanes themselves are no longer an unknown: all **94** AU6 tests pass on `windows-latest` (run 34661190014), so every gated row clears its budget on the `System233` FFmpeg package too — including the four rows at 2.03–2.22×, the `f64` seam's exact zero and the 31-band exact pin, the three §14 named as most exposed. What is left is bookkeeping: `cargo test` captures a passing test's stdout, so the margin **values** are not in the CI log. One `--nocapture` run per crate on a Windows checkout fills Appendix A row W (§13 item 5). A row whose Windows measurement differs from Linux's records **its own per-OS measured column**, measured and optimized there (AU3 §0 E59, §10.6 as amended).
- **The AU6 lane cost on Linux CI — paid.** The lever was sharing renders across fixtures, not cutting gates and not widening `AU6_MEDIA_LANE_BUDGET_SECONDS` / `AU6_AGENT_LANE_BUDGET_SECONDS` (still recorded, never gated, S16). `au6_fixtures.rs` now interns one `Au6Scene` per scenario plus the filled location pair, and a content-addressed render cache keys `mix_levels`, `mix_window_levels`, `mix_spectrum`, `audio_qc`, `audio_repair`, `mix_noise_profile`, `mix_audio_stems`, and delivery reports on `(kind, document JSON, request JSON)`. A mutated document is a different key — that is the invalidation rule. Historical cost (R23): `kinewright-media` lib **667 s** on Linux CI, job **8 m 58 s → 19 m 27 s**, AU6 ≈ **10.5 minutes**.
- **The Windows workspace job — §12's two cuttable fixtures and rust-cache, 2026-09-14.** `au6_b_the_makeup_less_chain_exceeds_the_loudness_range` and `au6_e_two_exports_at_one_profile_do_not_separate` are **cut**: the positive LRA and separation gates stay, the rows stay in `AU6_SOURCE_BUDGETS`, and probe-1's makeup-less **1 274** plus the 0-separation tautology sit in `reported_not_gated`. That is the §12 fallback (~21 s of encode). Beside it: `cc7_source` interns one FFV1 encode per `Cc7SourceKind::normalized` so the media lib does not re-mux the same 320×180 raster on every fixture, and Windows CI drops the redundant `cargo build --workspace` (`cargo test` already compiles the workspace; Linux keeps the extra build for the `readelf` soname gate). Both jobs use `Swatinem/rust-cache@v2` after `dtolnay/rust-toolchain`; the first run still compiles from scratch, later runs restore `target/`. No AU6 / CC7 / AU3 exit gate dropped; no budget widened.
...
- **`QueueExportArgs` without `deny_unknown_fields`.** **Paid.** `#[serde(deny_unknown_fields)]` on `QueueExportArgs` (`crates/kinewright-agent/src/server.rs`) emits `additionalProperties: false` on registry-only `queue_export` (+29 B input schema). AU6's cells send only known fields.
- **The duplicated `-6_000..=3_600` normalization gain guard.** **Paid.** `kinewright-media` publishes `NORMALIZATION_MINIMUM_GAIN_HUNDREDTHS` / `NORMALIZATION_MAXIMUM_GAIN_HUNDREDTHS`; `plan_audio_normalization` reads them. `au6_normalization_gain_guard_is_shared_with_export` pins the pair and forbids a second copy in `server.rs`. No ledger byte.
...
- **Retrofitting the `machine_assertions` string check to v1–v6** (§7.9; v5 g3's v9/v10 drift, N0.4(d)). **Paid.** `published_v1`–`published_v6` run the same non-empty / distinct / names-authority check as v7; integers ≥ 1000 in a published string must appear in the live definition or ground truth, which is the pin that refuses g3's leftover v9 aftermath window `1285..1345`. The g3 strings now track `music-ground-truth-v10` (title card at 388, story cut at 203, photographed close at 125). Product behavior is unchanged.
...
5. **The Windows lines.** The printed `AU6_*` margin lines are transcribed into Appendix A row W with the run id — recorded, never gated (N1 Q10). **Half discharged.** The lanes themselves now **pass on `windows-latest`**: 94/94 AU6 tests at `1f09c18`, CI run **34661190014** (§0.3 review finding 5), so every Windows margin cleared its budget and no AU6 claim is waiting on the operating system any more. What is still owed is the **figures**: `cargo test` captures a passing test's stdout, so the margin lines are not in the log. Transcribing them needs one `cargo test -p kinewright-media --lib au6_ -- --nocapture` (and the core, agent and app equivalents) on a Windows checkout, pasted into row W with the run id. Until then row W records the pass and not the numbers.
...
- **Windows was unmeasured for every AU6 number, and one of them is an exact `assert_eq!` — this risk is now retired.** It read: no Windows measurement of any AU3 or AU5 audio lane exists in-tree, only "CI green" (N0.5(c)), with the most exposed rows being dialogue-over-bed at **2.03×**, true peak at **2.04×**, the seam's exact zero on an `f64`, and the **31-band exact pin**, whose FFT is scalar `f64` with cross-OS agreement deliberately unasserted (AU3 E24, N0.5(f)). **Run 34661190014 measured it:** all 94 AU6 tests pass on `windows-latest`, so the two thin ratios clear, the seam still reads exactly zero and the 31-band pin still holds **exactly** on a different FFmpeg package — the outcome the mitigation was written for. The printed worst band and the printed margins remain the diagnostic if a later build moves either, and a divergence now earns **that system's own measured column** rather than a widened constant (AU3 §0 E59).
...
| E8 | the 50 Hz artefact | — | **Paid (§13).** Was `hum_50_excess` **0 → 829** (planner) / **1 072** (media chain) / **911** (probe-2); a second `mains_hum_present`. Shoulder reference now excludes notched neighbours; 60 Hz drop + first-three harmonic gates unchanged | probe-1 §3.4, probe-2 §5; `hum_shoulder_hertz` | core + media AU5 fixture | gated: no 50 Hz finding after a 60 Hz-only cascade; both-families fixture keeps it |
...
| W | **Windows** — every gated row above | — | **every row cleared; the figures are not transcribed yet.** All **94** AU6 tests pass on `windows-latest` — core 13/13, agent 8/8, app 14/14, media 59/59 — at `1f09c18`, CI run **34661190014**, so every gated row above holds on the `System233` FFmpeg package as well as on the `mifi` one, including the seam's exact zero and the 31-band exact pin. The printed `AU6_*` margin **values** are still missing because `cargo test` captures a passing test's stdout; one `--nocapture` run on a Windows checkout fills them in (§13 item 5) | the CI job | `windows-latest`, `System233` FFmpeg | each Windows run; a row whose measurement differs from Linux's records **its own per-OS measured column**, measured and optimized there (AU3 §0 E59) |
