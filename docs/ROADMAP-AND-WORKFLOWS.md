...
| IN2 — Investigator sessions (contracts: `IN2-INVESTIGATOR-SESSIONS.md`, `IN2B-INCIDENT-PERSISTENCE.md`) | a default investigator harness and model in a per-user settings file with discovery, an off switch and per-project per-code mutes in the project document; headless sessions started per incident against the live project on a branch core, with turn, token and wall-time budgets, one per project, deduped by `(code, subject)`, capped at two app-wide, over a named **54**-code allowlist; typed proposals recorded by a new registry-only `propose_fix` capability from the session's own branch, rendered as **approval cards from typed proposals**, applied by the application on one approval through the live core with an undo entry; **the confirmation broker guards destructive tools** and is widened to all seven destructive operation variants; a production scripted driver; `IncidentState::Investigating`, with every end that is not a person's decision returning the incident to `Open` rather than silencing it; the first deterministic recoveries outside the three auto-apply rows. Part B: persistence, a cross-process timestamp, the older-reader gate, the panel-local sinks, the `Result<_, String>` seams, the subject label and the `media backend error: ` prefix | sessions start for at least six `Explain` codes and for none of the three auto-apply codes; at least three proposals are approved and applied with zero destructive operations and a measured zero confirmation requests beside one deliberately provoked and auto-rejected; each of the three budgets stops a scripted session at its own bound, reports its numbers on the incident and returns that incident to `Open` unsuppressed; **three** `Explain` codes gain a pressable recovery, so the no-harness fallback resolves **3 of 67** by button; the served quad is unchanged in all three value sites for the eighteenth consecutive measurement; Part B ([`IN2B-INCIDENT-PERSISTENCE.md`](IN2B-INCIDENT-PERSISTENCE.md) §12): fourteen clauses — sidecar round trip without save, prune, Investigating→Open with 0 sessions on load, stale proposals, version gate, seam-produced sessions over ≥2 newly reachable codes, nine panel notes + two modal notes, prefix gone, observed names, nineteenth served measurement, seven sidecar arms, unknown-code aggregate, loaded-open eligibility + 0 sessions for the seven new codes, injected wall origin |
| IN3 — Catalogue breadth and harness efficiency | Incident codes for offline and relink, unsupported codecs with a transcode proposal, export and delivery failures, loudness and QC failures, hostile media; task-scoped capability packs so a job loads only the schema it needs; efficiency telemetry on every completed job; **a range-scoped timeline view** (one image per time range: filmstrip, waveform, word labels and silence-gap cut candidates) as the agent's on-demand visual layer beside the word-level transcript, instead of frame dumps; **a post-render cut-boundary self-check** run on the rendered output before a result is presented (flash or jump at a cut, audio pop, caption hidden by an overlay, loudness and true peak measured), with a capped number of fix passes and any remainder reported to the person; **dialogue-cut correctness defaults** built into the cutting operations and planners — never cut inside a word, pad cut edges within a bounded window to absorb transcript drift, rank silence gaps by length as cut candidates, a short audio fade at every cut, captions composited last — each verified against existing M39/AU behaviour before it is added; transcripts kept verbatim at word level with fillers preserved | Each code has a pinned class and recovery; token, tool-call, and wall-time budgets per job are pinned and green; the M35/M36 delta is published; the timeline view answers a cut-point question in a pinned byte budget; the self-check catches each seeded defect (flash, pop, hidden caption, level jump) in a fixture and passes a clean render; each correctness default has a test that fails without it |
