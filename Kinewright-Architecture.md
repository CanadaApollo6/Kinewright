# Kinewright — Architecture & Build Path

> A native Windows and Linux video editor written in Rust that is, at its core, an **agentic harness for video editing**. The agent brain is whatever coding-agent CLI the user already pays for — Claude Code, Codex, Cursor's `cursor-agent`, OpenCode — spawned as a subprocess, exactly the way T3 Code drives coding agents. Kinewright's job is to hand that agent *eyes and hands for video*: its tools are the same edit operations the human uses. Think "T3 Code, but the tools are cuts, trims, effects, and exports."
...
├── crates/
│   ├── kinewright-core/        # document model, Operation set, Core actor, undo, serde
│   ├── kinewright-media/       # ffmpeg decode/encode, frame cache, wgpu compositor, audio out, export
│   ├── kinewright-agent/       # MCP server (tools), CLI harness drivers (Claude Code/Codex/...), session mgmt
│   └── kinewright-app/         # eframe/egui binary — wires it all together, the UI
```

- **`kinewright-core`** — *No ffmpeg. No GPU. No network. No egui.* Pure logic and types. Fully unit-testable in isolation. Defines `Document`, `Operation`, `Command`, `Event`, the `MediaEngine` and `AgentDriver` traits (as contracts), undo/redo, and project (de)serialization. **This is the keystone crate — build its types first; everyone depends on them.**
- **`kinewright-media`** — implements the `MediaEngine` trait using `ffmpeg-next` + `wgpu` + `cpal`. Decode, frame cache, compositing, audio playback, export.
- **`kinewright-agent`** — the harness: an MCP server exposing the Operation set + inspector tools, plus per-CLI drivers that spawn and speak to Claude Code / Codex / cursor-agent / OpenCode subprocesses.
- **`kinewright-app`** — the `eframe` binary. Timeline widget, preview panel, media bin, inspector, agent chat panel. Thin view over `Document`; dispatches Operations.
...
### Document model (`kinewright-core`)
...
## 5. The AI harness (the part that makes it Kinewright)

This is the actual product. The design follows the T3 Code model, verified against how it actually works: **do not implement model API clients, do not handle credentials.** The user already has an agent CLI installed and authenticated on their machine — Claude Code, Codex, `cursor-agent`, OpenCode — riding their existing subscription. Kinewright spawns that CLI as a subprocess and speaks its structured stdio protocol (e.g. Claude Code's `--input-format stream-json --output-format stream-json`; Codex and the others have equivalents). The CLI does the model calls, the auth, the token refresh, the context management. Kinewright never sees a key.
...
**Tools are served over MCP.** `kinewright-agent` runs an MCP server (the official Rust SDK, `rmcp`) exposing two tool families, and every driver attaches it to its CLI session (all four CLIs speak MCP):
...
- Restrict the CLI session to Kinewright's MCP tools — the video-editing agent has no business running Bash or editing files. Every CLI has a permission/allowlist mechanism; drivers must configure it.
...
**Licensing.** Kinewright is **GPLv3** (LICENSE in repo root). This is a deliberate simplification: it lets the Windows installer bundle a full GPL build of FFmpeg (x264 and friends included) with zero license gymnastics, where a permissive license would force an LGPL-only FFmpeg build and dynamic-linking care. Everything Kinewright links must be GPL-compatible — the Rust ecosystem's MIT/Apache norm satisfies this. Contributions land under GPLv3; no CLA, no copyright assignment.
...
2. **Once M0 compiles, split three ways against the trait stubs:**
   - Team A: `kinewright-media` (M1 → M4 pipeline, including audio).
   - Team B: `kinewright-agent` (M3: MCP server + Claude Code driver first; can develop against a fake in-memory MediaEngine for `get_frame_at`).
   - Team C: `kinewright-app` (egui widgets, driving Core with a fake MediaEngine until A lands).
