...
- Playback with sample-accurate A/V sync (the audio clock is the master; video never leads) and **multi-track audio mixing that matches the export mixdown to sample-level parity** — per-track gain, pan, mute and solo in a Mixer with per-track, bus and master meters that keeps playing while you mix; parametric EQ, soft-knee compression, gating and a true-peak limiter on editable bus and master chains with latency-compensated lookahead; a constant-power pan law; BS.1770-4 loudness metering (momentary, short-term, integrated, loudness range, true peak) in the Mixer, published by what has actually been heard; keyframed clip gain envelopes drawn as a rubber band on the clip, track gain and pan automation, and bus and master fader automation edited in the Mixer, all evaluated integer-exactly and identically in playback and export, surviving every trim, split, slip and speed change under a stated policy; and levels, spectra, and an evidence-only audio QC report the agent can measure through the same mix path
